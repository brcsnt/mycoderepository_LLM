...
class CampaignBot:
    ...
    def get_campaign_description(self, code):
        """Kampanya koduna karşılık gelen açıklama metnini döndürür"""
        try:
            # Kodun integer olup olmadığını kontrol edin
            if not code.isdigit():
                return "Geçersiz kampanya kodu. Kampanya kodu sadece rakamlardan oluşmalıdır."
            
            # Kodun büyük/küçük harf duyarlılığını kontrol edin
            campaign_row = self.df.loc[self.df['Kampanya Kodu'] == code]
            if not campaign_row.empty:
                return campaign_row['Açıklama Metni'].values[0]
            else:
                return None
...
def query_button_clicked(b):
    ...
    with output:
        ...
        if description:
            if len(description) > 30000:
                print("Bu kampanya metni 30.000 karakterden uzun olduğu için sorulara cevap veremiyorum.")
            elif description == "Geçersiz kampanya kodu. Kampanya kodu sadece rakamlardan oluşmalıdır.":
                print(description)
            else:
                print(f"Kampanya Açıklaması:\n{description}")
                global campaign_description
...
!pip install pandas openai ipywidgets pyyaml
...
# config.yaml dosyasını okuyalım
...
class CampaignBot:
    def __init__(self, config):
        ...
        self.df = pd.DataFrame()  # Boş bir DataFrame oluşturuyoruz
        self.load_excel(self.excel_path)  # Excel dosyasını yükle
        openai.api_key = self.api_key  # OpenAI API anahtarını ayarla
...
# Widget'ları görüntüle
display(start_button, code_input, query_button, question_input, answer_button, output)
...
class CampaignBot:
    ...
    def ask_question(self, description, question):
        """Kampanya açıklamasını kullanarak OpenAI API ile soruyu cevaplar"""
        try:
            # Kampanya metni uzunluk kontrolü
            if len(description) > 30000:
                return "Bu kampanya metni 30.000 karakterden uzun olduğu için sorulara cevap veremiyorum."

            # Soru içeriği kontrolü (en az 5 haneli ardışık rakam)
            if re.search(r'\d{5,}', question):
                return "Sorduğunuz soruda müşteri bilgisi içeriyor olabilir. Lütfen tekrar kontrol ediniz."
